...
function getResponses(query) {
  ...
  console.log(query);
  var url = 'http://1a9d5d38.ngrok.io'+'/solr/'+ config.core +'/browse?q='+query+'&wt=xml';
  ...
  xhttp.onreadystatechange = function() {
    if (this.readyState == 4 && this.status == 200) {
		...
		for (var i = 0; i < xmlDoc.getElementsByName("dc_title").length; i++){
			...
			var link = xmlDoc.getElementsByName("id")[i].childNodes[0].textContent; //has space in directory

			var docValue =	'<div class="card" style="margin:10px;"><div class="card-header">#'+(i+1)+'</div><div class="card-block" style="margin:4px;"><h4 class="card-title">' + title + '</h4><p class="card-text">Score: ' + score +'</p><a href="'+ link +'" class="btn btn-primary">View Page</a></div></div>';
